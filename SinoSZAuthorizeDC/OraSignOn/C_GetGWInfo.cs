	...
	public class C_GetGWInfo
	{
		...
		public static bool Add_Post(string gwmc, string gwms, string _dwid)
		{
			...
			SqlParameter[] _param = {
                                ...
                                new SqlParameter(":GWMC",SqlDbType.NVarChar),
                                new SqlParameter(":GWMS",SqlDbType.NVarChar)                           
                        };
		...
		public static bool Del_RoleFromPost(string postid, string roleid)
		{
			...
			SqlParameter[] _param = {
                                new SqlParameter(":GWID", SqlDbType.Decimal),
                                new SqlParameter(":JSID",SqlDbType.NVarChar)                  
                        };
		...
		public static DataTable Get_QVRightsOfPost(string _postid)
		{
			...
			_sb.Append(" select v.viewid,v.namespace,v.viewname,v.displayname from md_view v");
			_sb.Append(" join qx_jscxmxgxb  jsv on  v.viewid=jsv.viewid");
			_sb.Append(" join qx2_gwjsgxb gwjs on gwjs.jsid=jsv.jsid");
			_sb.Append(" where gwjs.gwid=:GWID");
			_sb.Append(" order by displayorder");
			DataTable _dt = new DataTable();
			using (SqlConnection cn = SqlHelper.OpenConnection())
			{
		...
		{
			StringBuilder _sb = new StringBuilder();
			_sb.Append("select a.*,zhtj_zzjg2.getdwmc(a.dwid) ZZJGMC from qx2_yhxx a where ");
			_sb.Append(" a.yhid in ( ");
			_sb.Append(" select t.yhid from qx_yhgwgxb t where t.gwid = :GWID");
			_sb.Append(" ) ");
			DataTable _dt = new DataTable();
			using (SqlConnection cn = SqlHelper.OpenConnection())
			{
