...
namespace SinoSZJS.CS.Tasks.Customs.TJS
{
    public class Task_TJS : Task_Base
    {
        ...
        private bool CreateFile(string clny, string _TJSDir)
        {
            //创建上报文件
            try
            {
                ...
                string _MainDir = _TJSDir + Path.DirectorySeparatorChar.ToString()+string.Format("{0}", DateTime.Now.ToString("yyyyMMdd"));
                ...
                for (int i = -1; i > -13; i--)
                {
                    DateTime _lsdate = _today.AddMonths(i);
                    if (_lsdate.Year > 2005)
                    {
                        //2005年之前的不生成,之后的数据生成。
                        string _fname = _MainDir + Path.DirectorySeparatorChar.ToString() +string.Format("JR0000{0}.DAT", _lsdate.ToString("yyyyMM"));
                        WriteFile(_fname, _lsdate);
                    }
                }
                return true;
            }
        ...
        private const string SQL_WriteFile = @"select zbdm||','||ny||','|| gbdm||','|| trunc(NVL(sl1,0))||','|| trunc(NVL(sl2,0))||','|| tqsj||',' ||bmdm from tj_zbjlb
                                                WHERE NY=:NY and exists (select zbdm from  tj_zbdm_for_tjs tjs where tjs.zbdm=tj_zbjlb.zbdm)
                                                ORDER BY ZBDM,GBDM ";
        /// <summary>
        /// 生成数据文件
        /// </summary>
        /// <param name="_fname"></param>
        /// <param name="_lsdate"></param>
        private void WriteFile(string _fname, DateTime _lsdate)
        {
            StreamWriter sr = null;
            string _index = _lsdate.ToString("yyyyMM");
            try
            {
                using (OracleConnection cn = OracleHelper.OpenConnection())
                {
                    ...
                    cmd.Parameters.Add(new OracleParameter(":NY", _index));
                    using (OracleDataReader myReader = cmd.ExecuteReader())
                    {
                        while (myReader.Read())
                        {
