...
namespace SinoSZMetaDataBase.Controller
{
        public class MC_GuideLine
        {
                public static List<MD_GuideLineFieldGroup> GetFieldGroupsFromMeta(string _metaStr)
                {
                        ...
                        {
                                _group.Fields.Sort(new MD_GLFieldName_Comparer());
                        }
                        _ret.Sort(new MD_GLFieldGroup_Comparer());
                        return _ret;
                }
                ...
                public static List<MD_GuideLineParameter> GetParametersFromMeta(string _metaStr)
                {
                        ...
                        _ret.Sort(new MD_GLParameter_Comparer());
                        return _ret;
                }
                ...
                public static List<MD_GuideLineDetailDefine> GetDetaiDefinelFromMeta(string _metaStr)
                {
                        ...
                        foreach (Match _m in _mc)
                        {
                                ...
                                if (_s3.Length > 3)
                                {
                                        ...
                                        string _qid = _s3[2];
                                        string _qcs = _s3[3];
                                        MD_GuideLineDetailDefine _gdd = new MD_GuideLineDetailDefine(_fname, _type, _qid, _qcs);
                ...
                public static string CreateMeta(IList<MD_GuideLineFieldGroup> _fieldGroupList, IList<MD_GuideLineParameter> _parameterList, IList<MD_GuideLineDetailDefine> _detailList)
                {
                        ...
                        {
                                _metastr.Append(string.Format("<MX>{0}:{1}:{2}:{3}</MX>\n",
                                        _detail.TargetFieldName, _detail.QueryDetailType, _detail.DetailMethodID, _detail.DetailParameterMeta));
                        }
                ...
                private static string CreateReTableType(MD_GuideLineParameter _param)
                {
                        ...
                        {
                                return string.Format("代码表[{0},{1},{2}]", _param.RefTableName, (_param.IncludeChildren) ? 1 : 0, _param.SelecteAllCode);
                        }
                        else
                        {
