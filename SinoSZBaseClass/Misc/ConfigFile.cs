...
namespace SinoSZBaseClass.Misc
{
    public class ConfigFile
    {
        ...
        private static Config_IcsChannel ics_Channel = Config_IcsChannel.None;
        public static Config_IcsChannel ICS_Channel
        {
            get
            {
                if (ics_Channel == Config_IcsChannel.None)
                {
                    ics_Channel = Config_IcsChannel.Tcp;
                    //通道类型
                    string _ics_Channel_str = ConfigurationManager.AppSettings["ICS_Channel"].ToUpper();
                    //对某些配置信息进行解析或修正			
                    if (_ics_Channel_str == "TCP")
                        ics_Channel = Config_IcsChannel.Tcp;
                    else if (_ics_Channel_str == "HTTP")
                        ics_Channel = Config_IcsChannel.Http;
                    else
                        throw new Exception("配置文件中的ICS_Channel，必须为tcp或http");
                }
                return ics_Channel;
            }
        ...
        public static bool TraceLog
        {
            get
            {
                string _ics_Compress_str = ConfigurationManager.AppSettings["TraceLog"] as string;
                if (_ics_Compress_str == null) return false;
                string s = _ics_Compress_str.Trim().ToUpper();
                if (s == "1" || s == "TRUE" || s == "ENABLE" || s == "YES")
                {
