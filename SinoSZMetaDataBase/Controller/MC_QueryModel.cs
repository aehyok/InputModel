...
using System.Data;
using SinoSZMetaDataBase.DataCompare;
...
namespace SinoSZMetaDataBase.Controller
{
	public class MC_QueryModel
	{
		...
		public static MDModel_QueryModel CreateQuery_ModelDefine(MD_QueryModel _queryModel)
		{
			...
			MDModel_Table _mtable = new MDModel_Table(_queryModel.QueryModelName, _queryModel.MainTable);
			_ret.MainTable = _mtable;


			foreach (MD_ViewTable _ctable in _queryModel.ChildTables)
			{
				MDModel_Table _ct = new MDModel_Table(_queryModel.QueryModelName, _ctable);
				_ret.ChildTableDict.Add(_ct.TableName, _ct);
			}

			//重建字段的别名
			int _aliasIndex = 0;
			_ret.MainTable.AliasDict.Clear();
			foreach (MDModel_Table_Column _mtc in _ret.MainTable.ColumnDict.Values)
			{
			...
			foreach (MDModel_Table _ctable in _ret.ChildTableDict.Values)
			{
				...
				_ctable.AliasDict.Clear();
				foreach (MDModel_Table_Column _mtc in _ctable.ColumnDict.Values)
				{
		...
		public static MDModel_Table_Column GetColumnDefineByName(MDModel_QueryModel _qv, string _tName, string _cName)
		{
			...
			if (_mt.ColumnDict.ContainsKey(_cName))
			{
				return _mt.ColumnDict[_cName];
			}
			else
			{
				return null;
		...
		private static void CreateFullComboTableColumn(MDQuery_Request QueryRequest, MDModel_QueryModel QueryModel, DataTable _ret)
		{
			...
			foreach (MDQuery_ResultTable _ctable in QueryRequest.ChildResultTables.Values)
			{
		...
		private static void CreateComboTableColumn(MDModel_QueryModel QueryModel, MDQuery_ResultTable _resultTable, DataTable _ret)
		{
			...
			int i = 1;
			foreach (MDQuery_ResultTableColumn _rc in _resultTable.ColumnsDict.Values)
			{
				string FieldName = _rc.TableName + "_" + _rc.ColumnName;
